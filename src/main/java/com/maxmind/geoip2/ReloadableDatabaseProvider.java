...
/**
 * Implementation of {@link DatabaseProvider} that allows the
 * {@link DatabaseReader} to be rebuilt when the database is updated.
 */
public class ReloadableDatabaseProvider implements DatabaseProvider {
	/**
	 * Implementation of {@link Callable} that returns <tt>true</tt> if the supplied
	 * database {@link File} exists or has been modified.
	 */
	...
	/**
	 * Construct a {@link ReloadableDatabaseProvider}. This constructor provides
	 * options to check if the database is available and also lazy initialisation.
	 * Lazy initialisation allows a client to start an application with no database
	 * file whilst this can be downloaded in another thread.
	 *
	 * @param builder
	...
	public ReloadableDatabaseProvider(DatabaseReader.Builder builder, Callable<Boolean> availableChecker,
			boolean lazyInitialization) throws IOException {
	...
	/**
	 * Construct a {@link ReloadableDatabaseProvider} which initialises its reader
	 * eagerly.
	 *
	 * @param builder
	 * @param databaseFile
	...
	/**
	 * Construct a {@link ReloadableDatabaseProvider}. This constructor provides a
	 * lazy initialization option, that allows a client to start an application with
	 * no database file whilst this can be downloaded in another thread.
	 *
	 * @param builder
	 * @param databaseFile
	 * @param lazyInitialization
	...
	public AsnResponse asn(InetAddress ipAddress) throws IOException, GeoIp2Exception {
		return getProvider().asn(ipAddress);
	}
